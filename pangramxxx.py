...
def yarat(n):
    global llist, deneme, mutoran
    print("%s kelimelik denemeyle başlanıyor:" % n)
    print("Her turda nesil sayısı: %s, Mutasyon oranı: %f" % (deneme, (1/mutoran)))
    ilkgenler = random.sample(range(0,llist), int(n*2))
    ana = ilkgenler[0:int(n)]
    baba = ilkgenler[int(n):int(n*2)]
    print ("ilk anne:", yazdir(ana))
    print ("ilk baba:", yazdir(baba))
    print ("...")
    return(ana, baba, n)
...
if len(is_pangram(testyazi)) > 0:
    ...
    sayac = 0
    ana, baba, n = yarat(4)
    yana, ybaba, yn = nesil(ana, baba, n)
    while yana != True:
        yana, ybaba, yn = nesil(yana,ybaba,yn)
...
>>>>>>> 60588fb0597145992e746607913d5980c1ad49e4
